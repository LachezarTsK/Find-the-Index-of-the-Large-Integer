...
var getIndex = function (reader) {
    ...
    let firstLeft = 0;
    let firstRight = Math.floor((length - 1) / 2);

    let secondLeft = firstRight + balanceIndexForRangeWithOddLength(length);
    let secondRight = length - 1;

    let compare = reader.compareSub(firstLeft, firstRight, secondLeft, secondRight);
...
// return -1 if sum(arr[l..r]) < sum(arr[x..y])
    /*@param {number} l, r, x, y
     @return {number}
     */
    this.compareSub = function (firstLeft, firstRight, secondLeft, secondRight) {};
