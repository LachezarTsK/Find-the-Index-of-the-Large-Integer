...
class Solution {
    ...
    fun getIndex(reader: ArrayReader): Int {
        ...
        var firstLeft = 0
        var firstRight = (length - 1) / 2

        var secondLeft = firstRight + balanceIndexForRangeWithOddLength(length)
        var secondRight = length - 1

        var compare = reader.compareSub(firstLeft, firstRight, secondLeft, secondRight)
     ...
     */
    private fun balanceIndexForRangeWithOddLength(length: Int): Int {
        return (length % 2) xor 1
    }
}
